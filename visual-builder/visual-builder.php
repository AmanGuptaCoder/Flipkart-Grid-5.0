...
function mfnvb_updateVbView(){
	...
	if( isset($sections) && count($sections) > 0 ){
		...
		if(isset($mfn_styles['sidemenus']) && count($mfn_styles['sidemenus']) > 0){
			update_post_meta( $post_id, 'mfn-template-sidemenu', json_encode( array_unique($mfn_styles['sidemenus'])) );
		}else{
...
function mfnvb_converttoglobalsection(){
	...
	$be_content = wp_slash( $sections );

	update_post_meta($new_id, 'mfn_template_type', 'section');

	$mfn_styles = Mfn_Helper::preparePostUpdate($sections, $new_id);

	if(isset($mfn_styles['fonts']) && count($mfn_styles['fonts']) > 0){
		update_post_meta( $new_id, 'mfn-page-fonts', json_encode($mfn_styles['fonts']) );
	}else{
		delete_post_meta( $new_id, 'mfn-page-fonts' );
	}

	if( count($mfn_styles) > 0 ){
		$styles = $mfn_styles;
		unset($styles['sections']);
		update_post_meta( $new_id, 'mfn-page-local-style', json_encode($styles) );
		Mfn_Helper::generate_css($styles, $new_id);

		unset($styles);
		//print_r($mfn_styles);
	}else{
		delete_post_meta( $new_id, 'mfn-page-local-style' );
	}
	...
	update_post_meta($new_id, 'mfn-page-items', $be_content);
